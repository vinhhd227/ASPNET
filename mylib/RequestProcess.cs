        ...
        if (request.Method == "POST")
        {
            ...
            if (_form.Files.Count > 0)
            {
                ...
                foreach (IFormFile formFile in _form.Files)
                {
                    ...
                    {
                        var filePaths = "wwwroot/upload/" + formFile.FileName;    // Lấy tên  file
                        thongbaofile += $"{filePaths} {formFile.Length} bytes";
                        using (var stream = new FileStream(filePaths, FileMode.Create)) // Mở stream để lưu file, lưu file ở thư mục wwwroot/upload/
                        {
